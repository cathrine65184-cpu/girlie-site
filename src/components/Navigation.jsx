...
export function Navigation({ onJump, onOpenHall, onCreate, onOpenPrivateRoom, studioHref }) {
  ...
  return <header className="journey-nav">
    ...
    <div className="nav-right">
      ...
      <button className="studio-button" onClick={onOpenPrivateRoom}>{t('privateHouse')}</button>
      <LocaleMenu />
    ...
    {mobileOpen && <div className="mobile-halls" aria-label="Museum collections">
      ...
      <button onClick={onOpenPrivateRoom}>{t('privateHouse')}</button>
      <a className="mobile-studio" href={studioHref}>{t('buildSecretHouse')}</a>
    </div>}
  </header>;
}
