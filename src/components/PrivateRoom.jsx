...
export function PrivateRoom({ open, onClose, store, onStartInterview }) {
  ...
  return <AnimatePresence>{open && <motion.aside className="private-room-overlay" role="dialog" aria-modal="true" aria-label="My friendship archives" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <button className="private-backdrop" aria-label="Close friendship archives" onClick={onClose} />
